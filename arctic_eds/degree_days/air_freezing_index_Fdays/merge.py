...
def merge(directory_path):
    ...
    ds = xr.Dataset(
        data_vars={"air_freezing_index_Fdays": (dim_names, out_arr)},
        coords={
    ...
    with rio.open(
        f"ncar_12km_{models[2]}_{scenarios[1]}_air_freezing_index_2050_Fdays.tif"
    ) as src:
        test_arr = src.read(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process GeoTIFF files to create a combined NetCDF air_freezing_index_Fdays.nc."
    )
    parser.add_argument(
        "-d",
        "--directory",
