...
def merge(directory_path):
    output_path = os.path.join(os.getcwd(), "air_thawing_index_Fdays.nc")
    ...
    ds = xr.Dataset(
        data_vars={"air_thawing_index_Fdays": (dim_names, out_arr)},
        coords={
    ...
    with rio.open(
        f"ncar_12km_{models[2]}_{scenarios[1]}_air_thawing_index_2050_Fdays.tif"
    ) as src:
        test_arr = src.read(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process GeoTIFF files to create a combined NetCDF air_thawing_index_Fdays.nc."
    )
    parser.add_argument(
        "-d",
        "--directory",
