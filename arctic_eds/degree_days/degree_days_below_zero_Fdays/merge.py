...
def merge(directory_path):
    output_path = os.path.join(os.getcwd(), "degree_days_below_zero_Fdays.nc")
    ...
    ds = xr.Dataset(
        data_vars={"degree_days_below_zero_Fdays": (dim_names, out_arr)},
        coords={
    ...
    with rio.open(
        f"ncar_12km_{models[3]}_{scenarios[2]}_degree_days_below_zero_2099_Fdays.tif"
    ) as src:
        test_arr = src.read(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process GeoTIFF files to create a combined NetCDF degree_days_below_zero_Fdays.nc."
    )
    parser.add_argument(
        "-d",
        "--directory",
