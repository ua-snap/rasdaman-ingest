...
os.chdir("/tmp/degree_days/heating_degree_days_Fdays")
...
model_scenario_year_pattern = re.compile(
    r"ncar_12km_(.*?)_(.*?)_heating_degree_days_(.*?)_Fdays.*$"
)
...
for i, file in enumerate(tqdm.tqdm(files)):
    ...
    data_array = xr.DataArray(
        ...
        name="heating_degree_days_Fdays",
    )
...
ds.to_netcdf("heating_degree_days_Fdays.nc")
