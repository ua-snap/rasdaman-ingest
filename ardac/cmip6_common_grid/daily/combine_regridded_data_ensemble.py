...
NOTE: This script will allow you to combine all variables + models + scenarios into a single file, 
but it doesn't actually work on the complete dataset. Instead, for each variable combine historical 
or future scenario data separately. These can files can merged later (TBD) or ingested into rasdaman separately.
...
example usage:
  python combine_regridded_data_ensemble.py \
    ...
    --rasda_dir /beegfs/CMIP6/jdpaul3/cmip6_regrid_for_rasdaman
"""
...
def reindex_and_rechunk(ds, chunks):
    ...
    print("Dataset opened and combined successfully...")
    #print(ds.chunks)
...
if __name__ == "__main__":
    ...
    with Client(
        n_workers=6,  # 6 workers × 4 threads = 24 cores (28 cores available on the node)
        threads_per_worker=4,
        memory_limit="20GB",  # 6 × 20GB = 120GB (128GB available), leaves some headroom for OS/other processes
        ...
        # combine scenarios as "_" separated string
        scenario_str = "_".join(sorted(scenarios))

        out_fp = rasda_dir / f"cmip6_regrid_{frequency}_{var_str}_{scenario_str}_ensemble.nc"
