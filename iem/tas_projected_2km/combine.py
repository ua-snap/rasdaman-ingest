...
data_dir = "/usr/local/data/torgerso/temperature/tas"

# Loop through each model and month combination
for model in models:
    for month in months:
        ...
        tas_file = os.path.join(data_dir, f"tas_{model}_{month}_temperature.nc")
        tasmax_file = os.path.join(data_dir, f"tasmax_{model}_{month}_temperature.nc")
        tasmin_file = os.path.join(data_dir, f"tasmin_{model}_{month}_temperature.nc")
        ...
        # Create a new dataset with the concatenated variables
        ...
        output_filename = os.path.join(
            data_dir, f"combined_{model}_{month}_temperature.nc"
        )
